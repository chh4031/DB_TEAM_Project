...
const partnersignupView2 = async(req, res) => {
    ...
    if (existingCheckno.length < 0) {
        ...
        const insertPartnerQuery = 'INSERT INTO 제휴매장 (사업자번호, 제휴매장이름, 제휴매장분류, 제휴매장담당자, 제휴매장전화번호, 제휴매장주소, 제휴매장계약활성, 제휴매장평점, 제휴매장리뷰갯수, 제휴매장요청유무, 회원_아이디) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        await useDB.query(insertPartnerQuery, [partnerno, partnername, partnercategory, partnerowner, ownertel, partneraddress, parteneragree, partnerlating, latingcount, partnerreq, Id]);
        res.redirect('/');
    }
}
